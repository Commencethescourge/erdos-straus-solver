...
def patched_main():
    ...
    total = len(unsolved)
    print(f"[cpu-rescue3] {total:,} values, step_cap={cap:,}, workers={workers}")

    fields = ["n", "x", "y", "z", "steps", "solved", "run_id"]
    out_path = "cpu_rescue3_results.csv"
    solved_count = 0
    t0 = time.time()
    ...
    print(f"\n[cpu-rescue3] Done in {total_time:.1f}s")
    print(f"[cpu-rescue3] Solved: {solved_count:,} / {total:,}")
